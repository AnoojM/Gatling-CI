...
class BasicGetBusDetails_SubsequentRequest extends Simulation{
  ...
  val scn = scenario("GetBusDetails")
    ...
    .exec(http("getSeatlayoutrequest").get("/api/bus/seatmap/?app_id="+appId+"&app_key="+appKey+"&format=json&skey=${searchKey}")
    .check(bodyString.saveAs("seatLayoutResponse")))

    //.check(jsonPath("$data..skey").find.saveAs("searchKey"))
    .exec(session=>
      {
        println(session("seatLayoutResponse").as[String])
        session
      })
